defmodule RumblWeb.AuthTest do
  ...
  test "authenticate_user/2, halts when no current_user exist", %{conn: conn} do
    conn = Auth.authenticate_user(conn, [])
    assert conn.halted
  end

  test "authenticate_user/2, for exist current_user", %{conn: conn} do
    conn =
      conn
  ...
  test "call/2, with no session sets current_user assign to nil", %{conn: conn} do
